...
function PauseScreen({title,msg,onCont,locked}){return<Ph title=""><div style={{textAlign:"center",padding:"50px 20px",background:`linear-gradient(135deg,${C.dk},${C.dk2})`,borderRadius:22,color:"#fff"}}><div style={{fontSize:56,marginBottom:14}}>{locked?"⏸️":"✅"}</div><h2 style={{fontFamily:F.d,fontSize:26,margin:"0 0 10px",fontWeight:800}}>{title}</h2><p style={{color:"#a0aec0",fontSize:14,maxWidth:480,margin:"0 auto 24px",lineHeight:1.5}}>{msg}</p>{locked?<div style={{background:"rgba(255,255,255,.1)",borderRadius:10,padding:"14px 22px",display:"inline-block"}}><div style={{fontSize:13,color:"#fbbf24",fontWeight:700}}>🔒 Waiting for teacher to unlock...</div><div style={{fontSize:11,color:"#94a3b8",marginTop:3}}>Progress saved. Teacher will discuss first.</div></div>:<button onClick={onCont} style={{...bp,background:"linear-gradient(135deg,#fbbf24,#f59e0b)",color:C.dk,fontSize:15,padding:"12px 32px"}}>Continue →</button>}</div></Ph>}

// PHASE: Welcome
function Welcome({onStart,onTeacher}){const[sh,sSh]=useState(false);useEffect(()=>{setTimeout(()=>sSh(true),80)},[]);return<div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"linear-gradient(160deg,#0a0e27,#1a2744 40%,#0a3d2e)",fontFamily:F.b,padding:24,opacity:sh?1:0,transition:"opacity .7s"}}><div style={{fontSize:60,marginBottom:10}}>💸</div><h1 style={{fontFamily:F.d,fontSize:"clamp(36px,6vw,56px)",fontWeight:900,color:"#e8f5e9",textAlign:"center",margin:0,lineHeight:1.05}}>Life & Money</h1><p style={{color:"#80cbc4",fontSize:16,textAlign:"center",maxWidth:520,margin:"12px 0 32px",lineHeight:1.5}}>Navigate financial decisions — education, careers, housing, investing, and retirement.</p><div style={{display:"flex",gap:12,flexWrap:"wrap",justifyContent:"center"}}><button onClick={onStart} style={{...bp,padding:"13px 30px",fontSize:15}}>🎮 Start Journey</button><button onClick={onTeacher} style={{...bs,background:"rgba(255,255,255,.08)",color:"#a5d6a7",borderColor:"rgba(255,255,255,.15)"}}>🍎 Teacher Dashboard</button></div></div>}
// PHASE: Interests
...
function RetirementPhase({onDone,pd}){const sal=pd.salary||50000;const rp=pd.retPct||.06;const s=ps(sal,rp);const mo401=s.r4;const moInv=pd.monthlyInv||200;const ar=pd.avgRet||.08;const yrs=40;const r401=mo401*((Math.pow(1+.07/12,yrs*12)-1)/(.07/12));const rInv=moInv*((Math.pow(1+ar/12,yrs*12)-1)/(ar/12));const boost=pd.teacherAcctBoost||0;const ssMo=Math.min(3600,Math.round(sal*.4/12));const nest=r401+rInv+boost;const safe=nest*.04/12;const retMo=safe+ssMo;const ok=retMo>2500;
return<Ph title="🏖️ Retirement" sub={`After ${yrs} years of decisions.`}>
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(175px,1fr))",gap:10,marginBottom:16}}>
<SC icon="🏦" label="401(k)" value={fmt(Math.round(r401))} sub={`${fD(mo401)}/mo @ ${Math.round(rp*100)}%`} vc={C.g}/>
<SC icon="📈" label="Investments" value={fmt(Math.round(rInv))} sub={`${fmt(moInv)}/mo × ${yrs}yr`} vc={C.b}/>
