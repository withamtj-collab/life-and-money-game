...
// PHASE: Housing — includes home insurance + car insurance with deductible
...
const[homeIns,sHI]=useState("y");const[carIns,sCI]=useState("full");const[carDed,sCD]=useState(1000);
...
const go=()=>{setPd(d=>({...d,neighborhood:hood,homeSize:size,homePrice:price,downPay:da,mortPay:Math.round(totMo),hasHomeIns:homeIns==="y",homeInsMo:hiMo,carInsType:carIns,carDeductible:carDed,carInsMo:carMo,intRate:rate}));onDone()};
...
{size&&<div style={{display:"flex",justifyContent:"flex-end"}}><button onClick={()=>sS(2)} style={bp}>Mortgage & Insurance →</button></div>}</Fd>}
...
<div style={{background:C.card,borderRadius:14,padding:16,border:`1px solid ${C.br}`}}>
<div style={{fontSize:32,textAlign:"center",marginBottom:4}}>🏠</div>
<h4 style={{fontFamily:F.b,color:C.dk,margin:"0 0 4px",fontSize:13,textAlign:"center"}}>Homeowner's Insurance</h4>
<p style={{fontSize:10,color:C.tl,marginBottom:10,textAlign:"center"}}>Covers fire, theft, storms, liability. Est. {fD(Math.round(price*.004/12))}/mo.</p>
<div style={{display:"flex",gap:6,justifyContent:"center"}}><button onClick={()=>sHI("y")} style={{...(homeIns==="y"?bp:bs),padding:"6px 14px",fontSize:11}}>Yes {homeIns==="y"?"✓":""}</button><button onClick={()=>sHI("n")} style={{...(homeIns==="n"?{...bs,borderColor:C.r,color:C.r}:bs),padding:"6px 14px",fontSize:11}}>Skip</button></div>
{homeIns==="n"&&<div style={{fontSize:9,color:C.r,marginTop:6,textAlign:"center"}}>⚠️ One storm could cost $10,000+</div>}
</div>

<div style={{background:C.card,borderRadius:14,padding:16,border:`1px solid ${C.br}`}}>
<div style={{fontSize:32,textAlign:"center",marginBottom:4}}>🚗</div>
<h4 style={{fontFamily:F.b,color:C.dk,margin:"0 0 4px",fontSize:13,textAlign:"center"}}>Auto Insurance</h4>
<p style={{fontSize:10,color:C.tl,marginBottom:8,textAlign:"center"}}>KY requires liability. Full coverage protects your car too.</p>
<div style={{display:"flex",gap:4,justifyContent:"center",flexWrap:"wrap",marginBottom:8}}>
{[{v:"full",l:"Full Coverage"},{v:"liability",l:"Liability Only"},{v:"none",l:"None"}].map(o=><button key={o.v} onClick={()=>sCI(o.v)} style={{...(carIns===o.v?bp:bs),padding:"5px 10px",fontSize:10}}>{o.l}</button>)}
</div>
{carIns==="full"&&<div>
<div style={{fontSize:10,color:C.dk,fontWeight:600,marginBottom:4}}>Deductible: {fmt(carDed)}</div>
<input type="range" min={500} max={3000} step={500} value={carDed} onChange={e=>sCD(+e.target.value)} style={{width:"100%",accentColor:C.b}}/>
<div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:C.tl}}><span>$500 (higher rate)</span><span>$3,000 (lower rate)</span></div>
<div style={{fontSize:10,color:C.b,fontWeight:700,marginTop:4,textAlign:"center"}}>Monthly: {fD(carMo)}</div>
<div style={{fontSize:9,color:C.tl,marginTop:2,textAlign:"center"}}>Higher deductible = lower monthly but more out-of-pocket if accident</div>
</div>}
{carIns==="liability"&&<div style={{fontSize:10,color:C.tl,textAlign:"center"}}>Monthly: {fD(55)} — Covers others only, not your car.</div>}
{carIns==="none"&&<div style={{fontSize:9,color:C.r,textAlign:"center"}}>⚠️ Illegal in KY without at least liability!</div>}
</div></div>

<div style={{background:C.gl,borderRadius:12,padding:14,border:"2px solid #66bb6a",textAlign:"center",marginBottom:12}}>
<div style={{fontSize:11,color:C.tl}}>Total Monthly Housing + Insurance</div>
<div style={{fontSize:26,fontWeight:800,color:C.g,fontFamily:F.d}}>{fD(totMo+carMo)}</div>
<div style={{display:"flex",gap:12,justifyContent:"center",fontSize:10,color:C.tl,marginTop:4}}>
<span>Mortgage+Tax+HOA: {fD(totMo-hiMo)}</span>
{hiMo>0&&<span>Home Ins: {fD(hiMo)}</span>}
{carMo>0&&<span>Car Ins: {fD(carMo)}</span>}
</div>
<div style={{fontSize:10,color:C.tl,marginTop:4}}>{Math.round(((totMo+carMo)/ps(pd.salary).net)*100)}% of take-home{((totMo+carMo)/ps(pd.salary).net)>.35?<span style={{color:C.r,fontWeight:700}}> ⚠️ Above 35%!</span>:""}</div>
</div>
...
const PHASES=["welcome","interests","education","pause1","career","paystub","budget","pause2","lifeChoices","housing","pause3","investing","pause4","lifeEvents","retirement","summary","teacher"];
...
useEffect(()=>{if(pd.name&&phase!=="login"&&phase!=="welcome"&&phase!=="teacher"&&phase!=="teacherLogin"&&ready){saveStudent(pd);sSt(p=>{const e=p.find(s=>s.name===pd.name);return e?p.map(s=>s.name===pd.name?{...pd}:s):[...p,{...pd}]})}},[phase,ready]);

const go=(p)=>{sP(p);sPd(d=>{const u={...d,phase:p};if(d.name&&ready)saveStudent(u);return u});window.scrollTo({top:0,behavior:"smooth"})};
...
{phase==="lifeChoices"&&<LifeChoicesPhase onDone={()=>pd.wantsHouse?go("housing"):go("pause3")} pd={pd} setPd={sPd}/>}
{phase==="housing"&&<HousingPhase onDone={()=>go("pause3")} pd={pd} setPd={sPd}/>}
